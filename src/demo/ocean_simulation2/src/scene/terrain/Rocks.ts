...
export function createRocks(
  ...
  for (let i = 0; i < 1200 && rockInstances.length < 80; i++) {
    const x = (random() * 480 - 225) * 0.5,
      z = (random() * 255 - 135) * 0.5;
    const h = terrainHeight(x, z),
      d = coastDistance(x, z);
    const exposed = z < -17.5 || x < -72 || x > 86.5;
    if (exposed && d > -5.5 && d < 3 && h > -0.7) {
      const scale = (1.1 + (random() ** 1.8) * 4.2) * 0.5;
      addRock(x, z, scale);
      if (random() > 0.45) addRock(x + (random() - 0.5) * 3.5, z + (random() - 0.5) * 3.5, scale * 0.6);
    } else if (h > 12.5 && terrainSlope(x, z) > 0.75 && random() > 0.6) {
      addRock(x, z, (2.5 + random() * 4.5) * 0.5);
    }
  }
  ...
  const rocks = new THREE.InstancedMesh(rockGeometry, rockMaterial, Math.max(1, rockInstances.length));
  ...
  rocks.castShadow = enableShadows;
  rocks.receiveShadow = enableShadows;
