...
export function createTerrain(
  ...
  const rawCoastDistance = (x: number, z: number) => {
  ...
  const coastDistance = (x: number, z: number) => rawCoastDistance(x * 2, z * 2) * 0.5;

  const rawTerrainHeight = (x: number, z: number) => {
    const d = rawCoastDistance(x, z);
    if (d > 0) {
  ...
  const terrainHeight = (x: number, z: number) => rawTerrainHeight(x * 2, z * 2) * 0.5;

  const terrainSlope = (x: number, z: number) =>
    Math.hypot(terrainHeight(x + 0.75, z) - terrainHeight(x - 0.75, z), terrainHeight(x, z + 0.75) - terrainHeight(x, z - 0.75)) / 1.5;
  ...
  const terrainSize = 550;
  const terrainSegments = compact ? 192 : 256;
  ...
  shader.fragmentShader = shader.fragmentShader.replace(
    ...
    /* glsl */ `
      ...
      float focusedSunlight(vec3 world) {
        ...
        float wide = texture2D(uCausticWide, world.xz / 380.0 + vec2(0.5)).r;
        float fine = texture2D(uCausticDetail, detailUV).r;
