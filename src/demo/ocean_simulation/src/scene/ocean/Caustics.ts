...
export class Caustics {
  ...
  constructor(options: CausticOptions) {
    ...
    this.causticWide = createFloatTarget(compact ? 1024 : 1536, compact ? 1024 : 1536, true);
    this.causticDetail = createFloatTarget(1024, 1024, true);

    this.uniforms = {
      uCausticWide: { value: this.causticWide.texture },
      uCausticDetail: { value: this.causticDetail.texture },
      uCausticRegion: { value: new THREE.Vector3(40, 100, 120) },
    ...
    this.causticMaterial = new THREE.ShaderMaterial({
      uniforms: {
        ...
        uRegion: { value: new THREE.Vector3(0, 0, 760) },
        uSpacing: { value: 760 / (compact ? 256 : 448) },
      },
  ...
  update(refreshWide = true, ready = true) {
    ...
    this.causticMaterial.uniforms.uRegion.value.set(0, 0, 760);
    this.causticMaterial.uniforms.uSpacing.value = 760 / (this.compact ? 192 : 256);
    if (refreshWide) {
    ...
    if (this.uniforms.uCausticDetailActive.value > 0.001 || !ready) {
      ...
      this.causticMaterial.uniforms.uSpacing.value = 120 / (this.compact ? 256 : 448);
