  ...
  constructor(options: SpectralCascadeOptions) {
    ...
    this.transform = createSimulationMaterial(
      ...
      /* glsl */ `
        ...
        vec2 multiplyComplex(vec2 a, vec2 b) { return vec2(a.x * b.x - a.y * b.y, a.x * b.y + a.y * b.x); }
        void main() {
          vec2 cell = gl_FragCoord.xy - vec2(0.5);
    ...
    const clearMat = createSimulationMaterial({}, 'void main() { gl_FragColor = vec4(0.0); }');
...
export function buildSpectrumGLSL(fftResolutions: number[]): string {
  return /* glsl */ `
    ...
    vec3 waveDisplacement(vec2 p, float footprint, float attenuation) {
      ...
      vec3 swell = textureLod(uDisplacement0, swellCoordinates(p, c) / 1792.0, lod.x).xyz;
      ...
      vec3 sea = textureLod(uDisplacement1, windP / 211.0, lod.y).xyz;
      vec3 shortSea = textureLod(uDisplacement2, windP / 27.3, lod.z).xyz;
    ...
    vec2 waveSlopeFiltered(vec2 p, float footprint, float attenuation) {
      ...
      vec4 c = coastField(p);
      float depth = coastalDepth(p);
      vec2 swell = textureLod(uSlope0, swellCoordinates(p, c) / 1792.0, lod.x).xy;
      vec3 dis = textureLod(uDisplacement0, swellCoordinates(p, c) / 1792.0, lod.x).xyz;
      ...
      float steepening = (1.0 - smoothstep(2.0, 12.0, depth)) * sqrt(c.a);
      swell *= 1.0 + steepening * clamp((0.35 * dis.y - 0.30 * dot(dis.xz, vec2(0.886995, 0.461779))) / max(0.2, 0.64 * uSwellGain), -0.35, 0.8);
      swell = turnSwell(swell, c.yz) * swellEnvelope(depth, c.a) * energyRegion(p);
      vec2 windP = windCoordinates(p);
      vec2 sea = textureLod(uSlope1, windP / 211.0, lod.y).xy * mix(0.38, 1.0, c.a)
        + textureLod(uSlope2, windP / 27.3, lod.z).xy * mix(0.68, 1.0, c.a);
