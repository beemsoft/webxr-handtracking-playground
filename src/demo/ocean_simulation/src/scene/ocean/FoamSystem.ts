...
export class FoamSystem {
  ...
  constructor(options: FoamSystemOptions) {
    ...
    this.uniforms = {
      ...
      uFoamRegion: { value: new THREE.Vector3(40, 100, 128) },
      uFoamDetail: { value: 0 },
    };
    ...
    this.levels = [
      {
        ...
        region: new THREE.Vector3(0, 0, 760),
        elapsed: 0,
      ...
      /* glsl */ `
        ...
        vec4 previousAt(vec2 p) {
          ...
          if (max(abs(uv.x - 0.5), abs(uv.y - 0.5)) > 0.496) {
            if (uNear > 0.5) {
              result = texture2D(uFoamWide, clamp(p / 760.0 + vec2(0.5), vec2(0.001), vec2(0.999)));
            }
          } else {
        ...
        void main() {
          ...
          vec4 bed = texture2D(uHeightMap, p / 1100.0 + vec2(0.5));
          ...
          float stepSize = 1.8;
          vec2 grad = vec2(
            texture2D(uHeightMap, (p + vec2(stepSize, 0.0)) / 1100.0 + vec2(0.5)).r - texture2D(uHeightMap, (p - vec2(stepSize, 0.0)) / 1100.0 + vec2(0.5)).r,
            texture2D(uHeightMap, (p + vec2(0.0, stepSize)) / 1100.0 + vec2(0.5)).r - texture2D(uHeightMap, (p - vec2(0.0, stepSize)) / 1100.0 + vec2(0.5)).r
          ) / (2.0 * stepSize);
  ...
  update(delta: number, camera: THREE.Camera, target: THREE.Vector3, quality: number) {
    ...
    for (let i = 0; i < this.levels.length; i++) {
      ...
      if (i === 1) {
        level.region.set(Math.round(focusX / 8) * 8, Math.round(focusZ / 8) * 8, 128);
