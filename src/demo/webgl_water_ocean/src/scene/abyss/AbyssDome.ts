...
export const abyssDomeFragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    // Underwater sunlight forward scattering halo
    ...
    vec2 rayUv = vec2(dot(dir, rayTangent), dot(dir, rayBitangent)) / alongRay;
...
export function createAbyssDome(scene: Scene, sunDirection: Vector3): AbyssDomeController {
  const uniforms = {
    ...
    uShallowColor: { value: new Color(0x0e5d6d) },
    uDeepColor: { value: new Color(0x02161e) },
  };
  ...
  const geometry = new SphereGeometry(180, 48, 24);
